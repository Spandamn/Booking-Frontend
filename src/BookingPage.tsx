  ...
  const handleBookingSubmit = () => {
    ...
    fetch(`${config.apiBaseUrl}/bookSlot?roomName=${roomName}`, {
      ...
      .then(() => {
        ...
        setSelectedSlot(null);
        setEmail('');
        handleFetchSlots(); // Refresh slots after booking
      })
    ...
    <div className="booking-page container">
      ...
      {showSlots && selectedDate && (
        <div className="slots-container text-center">
          ...
          <div className="slots d-flex justify-content-center">
            {Array.from({ length: 16 }, (_, i) => i + 1)
              .filter(slot => selectedDate !== new Date().toISOString().split('T')[0] || slot + 7 >= currentHour)
              .map((slot) => (
                <div
                  key={slot}
                  className={`slot ${bookedSlots.includes(slot) ? 'booked' : selectedSlot === slot ? 'selected' : 'available'} m-2`}
